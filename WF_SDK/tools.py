...
from math import log10, sqrt, pi     # import necessary math functions
...
def spectrum(buffer, window, sample_rate, frequency_start, frequency_stop):
    ...
    #return spectrum
    #deleted above to give value in V not dbv
    return c_spectrum


"""------------------------"""
def spectrumphase(buffer, window, sample_rate, frequency_start, frequency_stop):
    """
        calculates the spectrum of a signal
    ...
    # get the spectrum
    spectrum_length = int(buffer_length / 2 + 1)
    c_spectrum = (ctypes.c_double * spectrum_length)()  
    c_spectrum_phase = (ctypes.c_double * spectrum_length)()
    ...
    frequency_start = max(frequency_start * 2.0 / sample_rate, 0.0)
    frequency_stop = min(frequency_stop * 2.0 / sample_rate, 1.0)
    dwf.FDwfSpectrumTransform(c_buffer, ctypes.c_int(buffer_length), c_spectrum, c_spectrum_phase, ctypes.c_int(spectrum_length), ctypes.c_double(frequency_start), ctypes.c_double(frequency_stop))
    spectrum = []
    for index in range(spectrum_length):
        spectrum.append(20.0 * log10(float(c_spectrum[index]) / sqrt(2)))
    #return spectrum
    #deleted above to give value in V not dbv
    for i in range(len(c_spectrum_phase)): 
        if False : 
            spectrum2[i] = 0  # mask phase at low magnitude
        else: c_spectrum_phase[i] = c_spectrum_phase[i]*180.0/pi # radian to degree
        if c_spectrum_phase[i] < 0 : c_spectrum_phase[i] = 180.0+c_spectrum_phase[i]

    return c_spectrum_phase